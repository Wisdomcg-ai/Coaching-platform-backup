...
export class ExcelExportService {
  ...
  private createExecutiveSummarySheet(): void {
    ...
    sheet.getCell('A1').value = `Financial Forecast - ${forecast.business_name || 'Business'}`
  ...
  private createAssumptionsSheet(): void {
    ...
    const operating = [
      ['COGS Percentage', forecast.cogs_percentage, '0.0%'],
      ['Growth Rate', forecast.growth_rate, '0.0%'],
      ['Seasonal Adjustment', forecast.seasonal_adjustment || 0, '0.0%']
    ]

    operating.forEach(([label, value, format]) => {
      sheet.getCell(`A${row}`).value = label
      sheet.getCell(`B${row}`).value = (value || 0) / 100
  ...
  private createPayrollSheet(): void {
    ...
    payrollEmployees.forEach(employee => {
      ...
      sheet.getCell(row, col++).value = employee.role || ''

      sheet.getCell(row, col).value = employee.base_salary || 0
      ...
      months.forEach(month => {
        ...
        const value = employee.forecast_months?.[monthKey] || 0
